...
def book_flight(intent_request):
    ...
    au_departing = try_ex(lambda: intent_request['currentIntent']['slots']['AU_Departing'])
    au_destination = try_ex(lambda: intent_request['currentIntent']['slots']['AU_Destination'])
    flight_date = try_ex(lambda: intent_request['currentIntent']['slots']['FlightDate'])
    departure_time = safe_int(try_ex(lambda: intent_request['currentIntent']['slots']['DepartureTime']))
    cabin_class = try_ex(lambda: intent_request['currentIntent']['slots']['CabinClass'])
    session_attributes = intent_request['sessionAttributes'] if intent_request['sessionAttributes'] is not None else {}
        ...
        if confirmation_status == 'Denied':
            ...
            if confirmation_context == 'AutoPopulate':
                return elicit_slot(
                    ...
                    {
                        'contentType': 'PlainText',
                        'content': 'What city would you like to depart from?'
                    }
                )
            ...
            if confirmation_context == 'AutoPopulate':
                if not departure_time:
                    return elicit_slot(
                        ...
                        {
                            'contentType': 'PlainText',
                            'content': 'What time do you want to depart? I can give you a early morning, late morning, early afternoon or evening flight.'
                        }
                    )
    ...
    return close(
        ...
        {
            'contentType': 'PlainText',
            'content': 'Thanks, I have booked your flight.'
        }
    )
    ...
    # Booking the flight.  In a real application, this would likely involve a call to a backend service.
    logger.debug('bookFlight under={}'.format(reservation))
    ...
    return close(
        ...
        'Fulfilled',
        {
            'contentType': 'PlainText',
            'content': 'Thanks, I have placed your reservation.   Please let me know if you would like to book a car '
                       'rental, hotel, or another flight.'
        }
    )
